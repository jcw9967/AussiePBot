		...
		else
		{
			...
			else if( attacker->client->pers.teamSelection == PTE_HUMANS )
			{
				if( ROTACAK_ambush_rebuild_time_temp < level.time )
				{
					...
					if( g_ambush_stage.integer == g_ambush_stageLimit.integer )
					{
						...
						AP( va( "cp \"^2>>EGGS HAVE SUICIDED<<^7\"" ) );
					}
					
					if( g_ambush_killsToNextStage.integer > 0 && level.numHumanClients * g_ambush_killsToNextStage.integer <= ROTACAK_ambush_kills )
					{
						ambush_next_stage();
					}
				}
	...
	vec3_t dir, vec3_t point, int damage, int dflags, int mod )
{
	...
	if( g_ambush_abuildDmg.integer <= 0 &&
		!targ->client && targ->biteam == BIT_ALIENS && mod != MOD_TARGET_LASER )
		return;
	else if( g_ambush_hbuildDmg.integer <= 0 &&
	...
	if( !dir )
		...
		VectorNormalize( dir );
	if( inflictor->client->pers.doubledmg == 1 && targ->client )
	{
		damage = (int)( damage * 2 );
	}
	...
	float radius, gentity_t *ignore, int mod, int team )
{
	...
	for( e = 0; e < numListedEntities; e++ )
	{
		...
		if( CanDamage( ent, origin ) || mod == MOD_GRENADE || points > 100 )
		{
...
void ambush_next_stage()
{
	char    string[1024];
	int     min, tens, sec;
	const int nextStage = g_ambush_stage.integer + 1;
	int     increment = g_ambush_rebuildTime.integer * 1000;

	ROTACAK_ambush_kills = 0;
	trap_Cvar_Set( "g_ambush_stage", va( "%d", nextStage ) );
	level.lastAmbushNextStageTime = level.time;
	
	if( g_ambush_stageSuicide.integer == 1 )
	{
		kill_aliens();
	}
	...
	AP( va( "cp \"^1--> Prepare for next wave (^2Stage %i^1) <--\n^1(attack at %s)^7\"", nextStage, string ) );
	AP( va( "print \"^1--> Prepare for next wave (^2Stage %i^1) <--\n^1(attack at %s)^7\n\"", nextStage, string ) );
