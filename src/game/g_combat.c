    ...
    else
    {
      ...
      else if( attacker->client->pers.teamSelection == PTE_HUMANS )
      {
        if( ROTACAK_ambush_rebuild_time_temp < level.time )
        {
          ...
          if( g_ambush_stage.integer == g_ambush_stage_limit.integer )
          {
            ...
            trap_SendServerCommand( -1, va( "cp \"^2>>EGGS HAVE SUICIDED<<^7\"" ) );
          }
          if( level.numHumanClients * g_ambush_killstonextstage.integer <= ROTACAK_ambush_kills )
            ambush_next_stage();
        }
