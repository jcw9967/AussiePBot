...
void MedGunFire( gentity_t *ent )
{
  ...
  if( traceEnt->health > 0 && traceEnt->health < traceEnt->client->ps.stats[ STAT_MAX_HEALTH ] )
  {
      traceEnt->health += MEDGUN_HEALRATE;
  }
}
...
void cancelBuildFire( gentity_t *ent )
{
  ...
  if( ent->client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS )
  {
        ...
        traceEnt->health > 0 )
    {
      ...
      {
        G_AddCreditToClient( ent->client, HBUILD_HEALRATE, qtrue);
...
void buildFire( gentity_t *ent, dynMenu_t menu )
{
  ...
  {
    if( ent->client->ps.stats[ STAT_MISC ] > 0 && g_nobuildtimer.integer <= 0)
    {
      G_AddEvent( ent, EV_BUILD_DELAY, ent->client->ps.clientNum );
      return;
    }
    ...
    if( G_BuildIfValid( ent, ent->client->ps.stats[ STAT_BUILDABLE ] & ~SB_VALID_TOGGLEBIT ) )
    {
      ...
      else if( ent->client->ps.stats[ STAT_PTEAM ] == PTE_ALIENS && !G_IsOvermindBuilt( ) && g_nobuildtimer.integer <= 0)
      {
        ent->client->ps.stats[ STAT_MISC ] +=
          BG_FindBuildDelayForWeapon( ent->s.weapon ) * 2;
      }
      else if( ent->client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS && !G_IsPowered( muzzle ) &&
          ( ent->client->ps.stats[ STAT_BUILDABLE ] & ~SB_VALID_TOGGLEBIT ) != BA_H_REPEATER && g_nobuildtimer.integer <= 0) //hack
      {
        ent->client->ps.stats[ STAT_MISC ] +=
          BG_FindBuildDelayForWeapon( ent->s.weapon ) * 2;
      }
      else
	if(g_nobuildtimer.integer <= 0)
	{
        ent->client->ps.stats[ STAT_MISC ] +=
...
===============
*/
gentity_t *G_FindNewZapTarget( gentity_t *ent )
{
  int       entityList[ MAX_GENTITIES ];
...
===============
*/
void G_UpdateZapEffect( zap_t *zap )
{
  int       j;
...
===============
*/
void G_CreateNewZap( gentity_t *creator, gentity_t *target )
{
  int       i, j;
  zap_t     *zap;
...
void FireWeapon( gentity_t *ent )
{
  ...
  switch( ent->s.weapon )
  {
    ...
    case WP_CHAINGUN:
      ...
      break;
    case WP_MINIGUN:
      bulletFire( ent, MINIGUN_SPREAD, MINIGUN_DMG, MOD_MINIGUN );
      break;
    case WP_FLAMER:
      flamerFire( ent );
      break;
