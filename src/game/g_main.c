...
static cvarTable_t   gameCvarTable[ ] =
{
  ...
  { &g_ambush_rebuild_time, "g_ambush_rebuild_time", "10", CVAR_ARCHIVE, 0, qfalse  },
...
void G_CalculateBuildPoints( void )
{
  ...
  //may as well pump the stages here too
  {
    ...
    if( g_humanStage.integer == S1 && g_humanMaxStage.integer > S1 )
      humanNextStageThreshold = (int)( ( g_ambush_stage_limit.integer / 3 ) * g_ambush_killstonextstage.integer * level.numHumanClients );
    else if( g_humanStage.integer == S2 && g_humanMaxStage.integer > S2 )
      humanNextStageThreshold = (int)( ( ( 2 * g_ambush_stage_limit.integer ) / 3 ) * g_ambush_killstonextstage.integer * level.numHumanClients );
    else
...
void G_CalculateStages( void )
{
  ...
  int ambushStageLimit = g_ambush_stage_limit.integer;
...
void CheckExitRules( void )
{
        ...
        ( level.numLiveAlienClients == 0 ) ) )
  {
    ...
    trap_SendServerCommand( -1, "print \"^2Humans win\n\"");
             ...
             ( level.numLiveHumanClients == 0 ) ) )
  {
    ...
    trap_SendServerCommand( -1, "print \"^1Aliens win\n\"");
