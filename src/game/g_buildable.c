		...
		if( self->s.groundEntityNum )
		{
				...
				self->s.origin2, BA_A_SPAWN, NULL ) ) != NULL )
			{
					...
					ent->s.eType == ET_MOVER )
				{
					...
					( ent->client->ps.stats[STAT_PTEAM] == PTE_ALIENS || ent->client->ps.stats[STAT_PTEAM] == PTE_HUMANS ) )
				{
...
void AHive_Think( gentity_t *self )
{
	...
	if( self->RecheckTime < level.time )
	{
		...
		;//AP(  va( "print \"Rechecking Hive, number of waypoints : %i\n\"", self->TotalWP));
		for( j = 1; j <= self->TotalWP; j++ )
		{
			while( j <= self->TotalWP && ( !Hive_CheckWP( self->r.currentOrigin, self->WPA[j - 1], self ) || ( !Hive_CheckWP( self->WPA[j - 1], self->WPA2[j - 1], self ) && self->inUse[j - 1] ) ) )
			{
				...
				self->TotalWP--;
				;//AP(  va( "print \"Removed invalid waypoint\n\""));
			}
		}
	}
	...
	if( self->spawned && !self->active && G_FindOvermind( self ) )
	{
	  //do some damage
		...
		for( i = 0; i < num; i++ )
		{
			...
			;//AP(  va( "print \"Target Detected\n\""));
			if( !Hive_CheckWP( self->r.currentOrigin, enemy->r.currentOrigin, self ) )
			{
				;//AP(  va( "print \"no LOS, checking waypoints\n\""));
				...
				if( TWP == -1 )
				{
					...
					else
					{
						...
						self->targWP = freeTWP;
						;//AP(  va( "print \"Secondary Assigned\n\""));
					}
				}
				else
				{
					self->targWP = TWP;
					;//AP(  va( "print \"Firing normally\n\""));
				}
				...
				if( self->targWP == -1 )
					...
					;//AP(  va( "print \"Found Waypoint, here it goes!\n\""));

			}
			else
			{
				;//AP(  va( "print \"We have LOS, record waypoint\n\""));
				if( self->TotalWP <= MAX_WPA )
				{
					...
					if( !visible )
					{
						...
						else
						{
							...
							self->TotalWP++;
							;//AP(  va( "print \"Waypoint Stored\n\""));
						}
					}
					else
						;//AP(  va( "print \"Unnessesary Waypoint, ignored\n\""));
				}
				else
					;//AP(  va( "print \"Out of Memory, ignored\n\""));
			}

			;//AP(  va( "print \"Launching the swarm!\n\""));
...
void G_BuildableThink( gentity_t *ent, int msec )
{
	...
	if( ent->biteam == BIT_ALIENS || ent->s.modelindex == BA_H_REPEATER )
	{
		ent->time1000 += msec;
...
itemBuildError_t G_CanBuild( gentity_t *ent, buildable_t buildable, int distance, vec3_t origin )
{
	...
	if( reason == IBE_NONE && ( tr2.fraction < 1.0 || tr3.fraction < 1.0 ) )
		return IBE_NOROOM;
	if( reason == IBE_NONE && G_BuildableRange( entity_origin, 100, BA_A_SPAWN ) &&
		ent->client->ps.stats[STAT_PTEAM] == PTE_HUMANS )
		return IBE_NOROOM;
...
int nearestPathNodeToLocation( const vec3_t location )
{
	float closestDistance;
	int closestDistanceFound = qfalse;
	...
	{
		nodeCoords[0] = level.paths[i].coord[0];
		nodeCoords[1] = level.paths[i].coord[1];
		nodeCoords[2] = level.paths[i].coord[2];

		distance = DistanceSquared( location, nodeCoords );

		if( !closestDistanceFound || distance < closestDistance )
		{
			closestDistance = distance;
			closestDistanceFound = qtrue;
			closestDistanceIndex = i;
		}
	}

	AP( va( "print \"closestDistanceIndex: %d\n\"", closestDistanceIndex ) );
...
static gentity_t *G_Build( gentity_t *builder, buildable_t buildable, vec3_t origin, vec3_t angles )
{
	...
	{
		built->nearestNodeIndex = nearestPathNodeToLocation( origin );
	}
...
void G_LayoutLoad( void )
{
	...
	layoutHead = layout = G_Alloc( len + 1 );
