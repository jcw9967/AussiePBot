...
static float PM_CmdScale( usercmd_t *cmd )
{
  ...
  if( pm->ps->stats[ STAT_PTEAM ] == PTE_HUMANS && pm->ps->pm_type == PM_NORMAL )
  {
    ...
    //naked human carrying minigun
    if( BG_InventoryContainsWeapon( WP_MINIGUN, pm->ps->stats ) && 
...
static void PM_CheckDuck (void)
{
  ...
  // iD bug? you can still crouch when you're a spectator
...
static void PM_Weapon( void )
{
  ...
  if( pm->ps->weaponstate == WEAPON_RELOADING )
  {
    if( maxClips > 0 )
    {
      ...
      if( pm->ps->stats[ STAT_STATE ] & SS_DOUBLEAMMO )
      {
		ammo*=2; 
      }
    }
  ...
  if( !BG_FindInfinteAmmoForWeapon( pm->ps->weapon ) && UNLIMITED_AMMO < 1)
  {
  ...
  if( pm->ps->weapon == WP_CHAINGUN || pm->ps->weapon == WP_MINIGUN )
  {
    if( pm->ps->pm_flags & PMF_DUCKED ||
        BG_InventoryContainsUpgrade( UP_BATTLESUIT, pm->ps->stats ) )
    {
...
void PmoveSingle( pmove_t *pmove )
{
  ...
  PM_GroundTrace( );
  // must update after every GroundTrace() - yet more clock cycles down the drain :( (14 vec rotations/frame)
  // update the viewangles
  PM_UpdateViewAngles( pm->ps, &pm->cmd );
